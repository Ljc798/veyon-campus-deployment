...
public static class ReadOnlyPreflight
{
    public static PreflightReport Check(PlanInput input)
    {
        ...
        checks.Add(os.Build >= 10240
            ? new("os", CheckLevel.Pass, $"检测到 {major}，系统构建 {os.Build}；具体兼容性仍需实机验收。")
        ...
        checks.Add(RuntimeInformation.OSArchitecture == Architecture.X64
            ? new("architecture", CheckLevel.Pass, "检测到 Windows x64。")
            : new("architecture", CheckLevel.Blocked, $"当前架构为 {RuntimeInformation.OSArchitecture}；本阶段仅计划支持 x64。"));
        checks.Add(new("computer", CheckLevel.Pass, $"当前计算机名：{Environment.MachineName}"));
        if (input.Operations.InstallVeyon)
        {
            input.Package!.VerifyUnchanged();
            checks.Add(new("public-key", CheckLevel.Pass,
                $"已重新读取部署包和 RSA 公钥，指纹 {input.Package.PublicKeyFingerprint[..12]}…"));
        ...
        checks.Add(new("privilege", CheckLevel.Unknown,
            "本阶段未检查管理员权限或执行器；预检结果不能作为执行授权。"));
        return new PreflightReport(DateTimeOffset.UtcNow, planSha256, packageSha256, checks);
    }
